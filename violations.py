...
account_sid = "AC74c89aaf781fd22b31958cb9f0df4570"
auth_token = "66c3bfd9b20082f3b5614b8347574d1b"
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            redline = int(height/3)
            start_point = (0, redline)
            end_point = (width, redline)
            cv2.line(im0, start_point, end_point, (0, 0, 255), 3)
                        ...
                        if names[c] in LABELS:
                            ...
                            if names[c] == 'Two-wheeler':
                                if y < redline:
                                    ...
                                    cv2.putText(im0, f"signal jump violation by: {number}", (x+5, y+15), cv2.FONT_HERSHEY_SIMPLEX , 0.5, color, 1)
                            ...
                            if names[c] == 'Two-wheeler':
                                ...
                                if head_count != helmet_count:
                                    cv2.imwrite('numberplate.jpg', im0[y-50:h, x:w])
                                    number = ocr('numberplate.jpg')
                                    cv2.putText(im0, f"no helmet violation by: {number}", (x+5, y+5), cv2.FONT_HERSHEY_SIMPLEX , 0.5, color, 1)
                                    client.api.account.messages.create(
                                                        to="+91-9019678950",
                                                        from_="+15709191651",
                                                        body=f"no helmet violation by: {number}")
                                    
                                if head_count > 2:
                                    cv2.imwrite('numberplate.jpg', im0[y-50:h, x:w])
                                    number = ocr('numberplate.jpg')
                                    cv2.putText(im0, f"triple riding violation by: {number}", (x+5, y+10), cv2.FONT_HERSHEY_SIMPLEX , 0.5, color, 1)
                                    client.api.account.messages.create(
                                                        to="+91-9019678950",
                                                        from_="+15709191651",
                                                        body=f"triple riding violation by: {number}")
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='test/video.mp4', help='file/dir/URL/glob/screen/0(webcam)')
